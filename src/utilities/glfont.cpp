...
Mesh generateTextGeometryBuffer(std::string text, float characterHeightOverWidth, float totalTextWidth) {
    ...
    const float uvWidth = 1.0f / 128.0f;;
    const float uvHeight = 1.0f;
    ...
    for(unsigned int i = 0; i < text.length(); i++)
    {
        ...
        const float baseU = text[i] / 128.0f;
        ...
        mesh.textureCoordinates.at(4 * i + 1) = {baseU + uvWidth, 0};
        mesh.textureCoordinates.at(4 * i + 2) = {baseU + uvWidth, uvHeight};

        mesh.textureCoordinates.at(4 * i + 0) = {baseU, 0};
        mesh.textureCoordinates.at(4 * i + 2) = {baseU + uvWidth, uvHeight};
        mesh.textureCoordinates.at(4 * i + 3) = {baseU, uvHeight};
